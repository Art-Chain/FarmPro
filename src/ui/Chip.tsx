import React, { useEffect } from 'react';
import { ViewProps } from 'react-native';

import { Typography } from '@/ui/common';
import { createStyle } from '@/features/utils';
import Animated, {
  Easing,
  interpolateColor,
  runOnJS,
  useAnimatedStyle,
  useSharedValue,
  withTiming
} from 'react-native-reanimated';
import { useTheme } from '@/features/themes';
import {
  Gesture,
  GestureDetector,
  GestureStateChangeEvent,
  TapGestureHandlerEventPayload
} from 'react-native-gesture-handler';

const AnimatedText = Animated.createAnimatedComponent(Typography);

const chipStyle = createStyle({
  paddingHorizontal: 10,
  paddingVertical: 4,
  borderRadius: 20,
  borderWidth: 1,
});

export interface ChipProps extends ViewProps {
  active?: boolean;
  onPress?: (event: GestureStateChangeEvent<TapGestureHandlerEventPayload>) => void;
  onPressIn?: (event: GestureStateChangeEvent<TapGestureHandlerEventPayload>) => void;
  onPressOut?: (event: GestureStateChangeEvent<TapGestureHandlerEventPayload>) => void;
}

export const Chip = React.forwardRef<Animated.View, ChipProps>(({
  active,
  children,
  onPress,
  onPressIn,
  onPressOut,
  ...props
}, fRef) => {
  const theme = useTheme();
  const pressed = useSharedValue(0);
  const activeValue = useSharedValue(0);

  const tap = Gesture.Tap()
    .onBegin((event) => {
      pressed.value = withTiming(1, {
        easing: Easing.elastic(1),
      });
      if (onPressIn) runOnJS(onPressIn)(event);
    })
    .onEnd((event) => {
      if (onPress) runOnJS(onPress)(event);
    })
    .onFinalize((event) => {
      if (onPressOut) runOnJS(onPressOut)(event);
      pressed.value = withTiming(0, {
        easing: Easing.elastic(1),
      });
    });

  useEffect(() => {
    activeValue.value = withTiming(active ? 1 : 0, {
      easing: Easing.elastic(1),
    });
  }, [active, activeValue]);

  const animatedChipStyle = useAnimatedStyle(() => ({
    backgroundColor: interpolateColor(
      pressed.value,
      [0, 5],
      ['transparent', theme.colors.primary.main],
    ),
    borderColor: interpolateColor(
      activeValue.value,
      [0, 1],
      [theme.colors.palette.gray[300], theme.colors.primary.main],
    ),
  }));
  const animatedTextStyle = useAnimatedStyle(() => ({
    color: interpolateColor(
      activeValue.value,
      [0, 1],
      [theme.colors.palette.gray[500], theme.colors.primary.main],
    ),
  }));

  return (
    <GestureDetector gesture={tap}>
      <Animated.View ref={fRef} {...props} style={[chipStyle, animatedChipStyle, props.style]}>
        <AnimatedText style={animatedTextStyle}>
          {children}
        </AnimatedText>
      </Animated.View>
    </GestureDetector>
  );
});
